  ...
  constructor(container) {
    ...
    this.cols = [
      ...
      container.querySelectorAll(
        ".follow__label, .teaching__title, .is-active .running__date, .is-active .notes__venue, .friends__name, .footer__nav a, .site__footer .label",
      ),
      container.querySelectorAll(
        ".follow__value, .teaching__venue, .is-active .running__distance, .is-active .running__time, .is-active .notes__title, .friends__comment, .site__footer .value",
      ),
      container.querySelectorAll(
        ".teaching .is-featured-container, .is-active .running__pace",
      ),
      container.querySelectorAll(
        ".home__running .is-active .is-featured-container",
      ),
    ];
  ...
  render() {
    this.cols.forEach((col, i) => {
      col.forEach((el, j) => {
        ...
        const x = Math.sin(
          (window.innerWidth / this.cols.length) * i * 5 + this.time.getDay(),
        );
        const v =
          this.min +
          Math.sin(step + this.time.getHours() + x * 0.7) * -this.amplitude;
